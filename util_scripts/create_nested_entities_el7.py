...
Usage:
./create_nested_entities.py [--host_ip=<host ip>]
  --base_cluster_name=<cluster_name> --num_nodes=<num nodes to create>
...
class Cluster(object):
  ...
  #JARVIS_SERVER = "jarvis-dev.eng.nutanix.com"
...
def upload_to_jarvis_database(data):
  ...
  TMP_JARVIS_SERVER = "jarvis.eng.nutanix.com"
  #TMP_JARVIS_PORT = 8002   
  TMP_NESTED_URL = "api/v2/nested_entities/"

  url = "https://%s/%s" % (TMP_JARVIS_SERVER, TMP_NESTED_URL)
...
class PartitionDisks(Util):
  ...
  def lsblk(self):
    """
    This function uses lsblk to list all the SCSI devices attached to this VM.
    ...
    """
    ret = []
    _, result, stderr = self.run_cmd_ssh('lsblk --scsi')
    lines = result.splitlines()
    for line in lines[1:-1]:
      words = line.split()
      dev = "/dev/" + words[0].strip()
      ret.append(dev)
  ...
  def get_disks_eligible_for_partitioning(self, min_disk_size_gb):
    ...
    devs = [dev for dev in devs
            if self.get_size(dev) > (min_disk_size_gb * GB)]
  ...
  def change_disk_permission(self, disk):
    """
    Change the permissions on the disk to 777.
    """
    cmd = "chmod 777 %s" % disk
    status, stdout, stderr = self.run_cmd_ssh(cmd)
    if status:
      print "Failed to change the permission on the disk %s" % disk
  ...
  def partition_disks(self, args):
    ...
    print "Creating partitions on disks %s " % disks
    ...
    for disk in disks:
      ...
      self.change_disk_permission(disk)
      ...
      for count in xrange(args.num_nodes):
        ...
        self.change_disk_permission(partition)
